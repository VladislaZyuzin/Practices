...
`git fetch` только получает информацию о том, что нового в удалённом репозитории, например, когда кто то из команды залил туда изменения в код, а мы о них не знаем, кароч, просто засланный казачок

В свою очередь `git pull` - выполняет скачивание материалов удалённой репы на локальную репу

Хорошая практика - сначала делать fetch, а потом pull 

**Что деалет флаг -u в команде push?**

`git push -u <имя_ветки>` Позволяет отправить ветку на сервер, а так же, в чём основное отличие от простого `git push` - данная команда устанавливает upstream связь и при следующих пушах можно использовать просто команду `git push` это удобно, но не очень безопасно.

**Когда лучше использовать rebase, а когда merge?**

Rebase лучше для локальной работы и очистки истории перед мержем, merge - для командной работы и интеграции фич. Rebase создает чистую линейную историю, merge сохраняет точную историку изменений.

**Что опасно в rebase для опубликованных коммитов?**

Rebase переписывает историю Git, создавая новые коммиты с новыми хэшами. Если эти коммиты уже были опубликованы, это вызывает расхождение репозиториев, может привести к потере работы коллег и требует принудительного push, что опасно для командной работы.
